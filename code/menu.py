...
class Menu:
    ...
    def create_buttons(self):
        ...
        self.palm_button_rect = generic_button_rect.copy().move(self.rect.height / 2,  self.rect.width /
                                                                2).inflate(-button_margin, -button_margin)
    ...
    def highlight_indicator(self, index):
        ...
        if current_item in ('palm bg', 'palm fg'):
...
class Button(pygame.sprite.Sprite):
    ...
    def get_current_item(self):
        return self.items['main' if self.main_active else 'alt']

    def get_id(self):
        return self.get_current_item()[self.index][0]

    def switch(self):
        self.index += 1
        self.index = 0 if self.index >= len(self.get_current_item()) else self.index

    def update(self):
        self.image.fill(BUTTON_BG_COLOR)
        surf = self.get_current_item()[self.index][1]
